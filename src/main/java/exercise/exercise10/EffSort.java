  ...
  */
    void heapSort() {/*Chuyen danh sach thanh HEAP bang cach xem khoi dau heap gom nut
      ...
      a[n-1] vao heap*/
        ...
        int x;
        for (i = 1; i < n; i++) {
            //gán x = giá trị tại node con đang xét, x sẽ làm giá trị trung gian khi a[i] đổi giá trị với cha nó
            ...
            */
            //s la nut con, hien tai tren heap chua co a[i]
            s = i;
            //f=(s-1)/2 la nut cha
            ...
            //nếu giá trị node con(s) lớn hơn giá trị tại node cha ((s - 1) / 2) thì
            while (s > 0 && x > a[(s - 1) / 2]) {
                a[s] = a[(s - 1) / 2]; //Keo nut < x xuong 1 muc
                s = (s - 1) / 2;//chỉnh lại chỉ số node con thành chỉ số node cha nó vừa đổi giá trị
            }
            //Dua nut x vao vi tri phu hop, gán lại giá trị node con ban đầu vào node cha cuối cùng nó hoán đổi
            a[s] = x;
        }
        ...
        //chạy từ cuối mảng về i = 1
        for (i = n - 1; i > 0; i--) {
            //cho x là giá trị mảng tại i, đây là lá cuối cùng của heap
            x = a[i];
            //đổi chỗ giá trị tại điểm đầu mảng heap với tại i, đây là root có giá trị lớn nhất
            a[i] = a[0];
            ...
            */
            ...
            //Nut a[k] duoc chen dung cho, khi giá trị tại node f không nhỏ hơn node con lớn nhất thì dừng lại
